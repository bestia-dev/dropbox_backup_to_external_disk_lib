...
pub(crate) fn random_seed_32bytes_and_string() -> Result<([u8; 32], String)> {
...
pub(crate) fn open_file_b64_get_string(plain_file_b64_path: &CrossPathBuf) -> Result<String> {
    if !plain_file_b64_path.exists() {
        return Err(Error::ErrorFromStr(
            "{RED}Error: File {plain_file_b64_path} does not exist! {RESET}",
        ));
    }
...
pub(crate) fn shorten_vec_bytes_to_32bytes(vec_u8: Vec<u8>) -> Result<[u8; 32]> {
    if vec_u8.len() < 32 {
        return Err(Error::ErrorFromStr("The bytes must never be less then 32 bytes."));
    }
...
pub(crate) fn encode64_from_32bytes_to_string(bytes_32bytes: [u8; 32]) -> Result<String> {
    Ok(<base64ct::Base64 as base64ct::Encoding>::encode_string(&bytes_32bytes))
}

/// Decode base64 from string to 32bytes.
pub(crate) fn decode64_from_string_to_32bytes(plain_seed_string: &str) -> Result<[u8; 32]> {
...
///
/// It is a silly little obfuscation just to avoid using plain text.
pub(crate) fn decode64_from_string_to_string(string_to_decode: &str) -> Result<String> {
...
fn sign_seed_with_ssh_agent(
    ...
    fn public_key_from_ssh_agent(
        ...
        let vec_identities = client
            .list_all_identities()
            .map_err(|err| Error::ErrorFromString(err.to_string()))?;
        ...
        Err(Error::ErrorFromStr("This private key is not added to ssh-agent."))
    }
...
fn sign_seed_with_private_key_file(
    ...
    fn user_input_secret_passphrase() -> Result<SecretString> {
        println!();
        ...
        if secret_passphrase.expose_secret().is_empty() {
            return Err(Error::ErrorFromStr("Passphrase empty"));
        }
...
pub(crate) fn encrypt_symmetric(secret_passcode_32bytes: SecretBox<[u8; 32]>, secret_string_to_encrypt: SecretString) -> Result<String> {
    // nonce is salt
...
pub(crate) fn decrypt_symmetric(secret_passcode_32bytes: SecretBox<[u8; 32]>, plain_encrypted_string: String) -> Result<SecretString> {
